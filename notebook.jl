...
begin
	using PlutoUI #PlutoのUIパッケージ
	using Statistics # 統計パッケージ
	using Combinatorics # 組み合わせパッケージ
	using Distributions #分布パッケージ
	using StatsPlots #統計プロットパッケージ
	using NLsolve #非線形方程式求解パッケージ
	using Printf
end
...
1,2,2,3,3の数字を記入した5枚のカードがある。この5枚のカードを母集団とする。

(1) カードの数字を変量とするとき，母平均 $\mu$ と母標準偏差 $\sigma$ を求めよ。

(2) 非復元抽出で大きさ2の標本 $X_1$ ，$X_2$ をとる。このとき標本平均 $\overline X=\dfrac{X_1+X_2}2$ の確率分布を求めよ。
...
let
	...
	μ , σ = mean(X) , std(X , corrected = false) #母平均 μ と母標準偏差 σ
...
let
	...
	X₁₂ =  permutations(X,2) |> collect　# （X₁，X₂)を作成

	X̄ = [(p[1]+p[2])/2 for p ∈ X₁₂] # (X₁+X₂)/2を作成
...
# ╔═╡ fbd99736-c205-4c1f-84cd-4675a96a4818
md"""

### ② 標本平均の平均，標準偏差

1個のサイコロを210回投げる。出る目の平均を$\bar X$とする。$\bar X$の平均と標準偏差を求めよ。

"""

# ╔═╡ b44cf2b5-9d41-4fa7-bc64-fa81a3ef424e
let
	X = [1,2,3,4,5,6]

	E_X̄ , σ_X̄ = mean(X) , std(X , corrected = false)/sqrt(210) 
	#標本平均の平均E(X̄) と標本平均の標準偏差 σ(X̄)

end

# ╔═╡ d7b2e285-7733-45e7-954a-9b4185adfe42
md"""
...
この高校2年生の数学のテストの母平均は52点，母標準偏差は16点であった。

この母集団の中から大きさ400の無作為標本を抽出するとき，標本平均$\bar X$が51点以上である確率を求めよ。


"""

# ╔═╡ 6bc86074-3557-47d7-97ed-7d09c12a2d62
let
	μ , σ , n = 52 , 16 , 400
...
# ╔═╡ 71974248-4c5e-4660-986e-0528ac5574fa
md"""

### ④ 母比率・標本比率

ある種子の発芽率（母比率）は40%である。

この種子を600粒まいたときの発芽率（標本比率）$R$が37%以上42%以下である確率を求めよ。

"""

# ╔═╡ 78b9409a-8430-44c7-b55f-2933639c9745
let
	p , n = 0.4 , 600

	R = Normal(p , sqrt(p*(1-p)/n))

	cdf(R , 0.42)-cdf(R , 0.37)

end

# ╔═╡ a2806096-9b5c-4f51-b37b-da3c96008c2e
let
	B = Binomial(600 , 0.4)

	cdf(B,0.42*600)-cdf(B,0.37*600)
	
end
	

# ╔═╡ 595c2451-a4ff-4c84-a9a3-8c4eaedb134b
let
	p , n = 0.4 , 600
	N = Normal(n*p , sqrt(n*p*(1-p)))
	B = Binomial(600 , 0.4)
	plot(B,label="Binomial($n,$p)")
	plot!(N,label="Normal($(n*p),$(n*p*(1-p)))")
	plot!(xlim=(180,300))
end


# ╔═╡ 92ca46bb-4064-41c7-8931-fb8a73d4a9d6
md"""

### ⑤ 推定

有権者が1万人いるという町で，2つの政策A，Bに対する支持を調べる世論調査を行なった。


(1) 政策Aに対する世論調査を任意に抽出した有権者1200人に対して行ったところ，支持は300人であった。この街の有権者1万人のうち政策Aの賛成者は何人くらいいると推定されるか？信頼度95％で推定せよ。

(2) 政策Bに対する支持率は約50%と予想されている。この政策に対する支持率を，信頼区間の幅が4%以下になるように推定したい。信頼度95%で推定するには，何人以上抽出して調べればよいか。

"""

# ╔═╡ 0996b33e-151e-4242-9c99-4c54b1c3b1be
let
	p , n = 300/1200 , 1200

	R = Normal(p , sqrt(p*(1-p)/n))

	a , b = quantile(R , 0.025)*10000 , quantile(R , 0.975) *10000

end

# ╔═╡ dcd9a870-43ec-41db-98c3-cec551ba72f3
let

	function confint(n) # 信頼区間
		p = 0.50
		R = Normal(p , sqrt(p*(1-p)/n))
		a , b = quantile(R , 0.025) , quantile(R , 0.975)
		b - a - 0.04
	end

	function nls(func, params...; ini = [0.0]) #非線形方程式の近似解を求める
...
# ╔═╡ 06aeada9-17d3-4e6a-9a5b-12c025cd1071
md"""
### ⑥ 推定・両側検定


ある工場で大量生産されている電球の中から無作為に抽出した64個について試験したところ，それらの平均寿命は1582時間であった。また，製品の寿命の標準偏差は120時間であることが知られている。

(1) 製品全体の平均寿命を信頼度95%で推定せよ。

(2) この製品は平均寿命が1600時間と表示されている。このとき，この製品1個あたりの平均寿命は表示通りでないと判断してよいか。有意水準5%で検定せよ。


"""

# ╔═╡ de03a822-f0b2-4aeb-b7c4-bfabf3fcb964
let
	n , μ , σ = 64 , 1582 , 120

	X = Normal(μ , σ/sqrt(n))

	a , b = quantile(X , 0.025) , quantile(X , 0.975)

end

# ╔═╡ e5da0dca-8e3a-458c-be82-6e6643b0a4b2
let
	# using Distributions
	# using Printf
	# using StatsPlots
...
# ╔═╡ 87efd2fc-4643-4c4b-be3d-f2148e3846bd
let
	function pvalue_ztest(X̄, n; μ, σ)
	    z = √n * (X̄ - μ) / σ
	    2ccdf(Normal(), abs(z))
...
# ╔═╡ baaea760-6d16-46fe-bdc4-d2d2561636c9
md"""

### ⑦ 片側検定

 ある会社の自社商品の満足度を調査したところ，80%のユーザーが満足しているという結果であった。商品の改良を行い，改めて満足度を調査したところ，400人のうち334人が満足していると答えた。この商品の満足度は高まったと判断してよいか有意水準5%で検定せよ。


"""

# ╔═╡ 57bb36ef-7519-4d16-90bc-c2cf7d8b3ead
let
	default(fmt=:png, titlefontsize=10)

		
	function p_value2(n,k,p)
	    z = (n*p -k)/sqrt(n*k/n*(1-k/n))
	    2ccdf(Normal(),abs(z))
	end

	
	function confint2(n,k, α=0.05)
	    c = cquantile(Normal(), α/2)
	    [k/n - c*sqrt(k/n*(1-k/n)/n), k/n + c*sqrt(k/n*(1-k/n)/n)]
	end
	...
	plot(p -> p_value2(n,k,p),.75,.95; label="P-value function", c=1)
	hline!([α]; label="", c=2, ls=:dot)
	plot!(ci, fill(α, 2); label="$(100(1-α))% CI = $(round.(1000 * ci)/1000)", lw=3, c=2)
	vline!([k/n]; label="point estimate = $(k/n)", c=3, ls=:dash)
	vline!([p]; label="", c=4, ls=:dashdot)
	scatter!([p], [pval]; label="P-value($p) = $(round(100pval; sigdigits=3))%", msc=:auto, c=4)
	plot!(ytick=:0:0.05:1, xtick=0:0.02:1)
	plot!(xguide="p", yguide="P-value")
	title!("P-value function of z-test for sample mean=$(k/n), n=$n, k=$k")
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "6a5cef2705cda6b909d54b9cf8ed0e498194b242"
